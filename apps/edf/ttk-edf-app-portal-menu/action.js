...
class action {
    ...
    load = async (page, entity = {}) => {
        ...
        const tree = await this.webapi.menu.queryAll()
        response.tree = tree
    ...
    delType = async () => {
        const treeData = this.metaAction.gf('data.treeData').toJS()
        ...
        if (ret) {
            const id = this.metaAction.gf('data.filter.parentId')
            let delData = treeData.filter(function (data) { return data.id == parentId })
            const response = await this.webapi.menu.deleteBatch(delData)
