...
class reducer {
    ...
    load = (state, response) => {
	    ...
	    state = this.metaReducer.sf(state, 'data.treeData', fromJS(response.tree))
        ...
        if (response.filter.isReloadTree === undefined || response.filter.isReloadTree === true) {
            state = this.metaReducer.sf(state, 'data.other.tree', fromJS(tree.build(response.tree).children))
        }
        return state
    }
