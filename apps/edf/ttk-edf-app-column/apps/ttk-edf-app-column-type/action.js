...
class action {
    ...
    onInit = ({ component, injections }) => {
        ...
        if(this.component.props.actionType == 'add') {
            ...
            this.metaAction.sf('data.modifyId', this.component.props.modifyId)
    ...
    save = async () => {
        ...
        form.isDefault = form.isDefault == true ? 1 : 0
        if (type == 'modify') {
            const response = await this.webapi.columnType.update(form)
            this.metaAction.toast('success', '修改成功')
            return response
        }else if(type == 'add'){
            const response = await this.webapi.columnType.create(form)
