...
class action {
    ...
    openAssignImportProgress = async (seqNo,res,sessionNo) => {
        const ret = await this.metaAction.modal('show', {
            title: '导入分配数据',
            className: 'ttk-es-app-customer-assign-import-progress-modal',
            ...
            centered:true,
            // okText:'终止下载',
            children: this.metaAction.loadApp('ttk-es-app-customer-assign-import-progress', {
    ...
    judgeChoseBill = async (type) => {
        ...
        if (type !== 'importAssign'&&
            ...
            const ret = await this.metaAction.modal('show', {
                ...
                cancelText: '取消',
                okText: '导入',
                children: this.metaAction.loadApp('ttk-es-app-customer-assign-import', {
                    store: this.component.props.store,
                    path: 'person'
                })
            });
    ...
    load = async () => {
        ...
        if (resp) {
            let list = [];
            console.log(resp.list,'我是列表')
            if(resp.list){
                list = resp.list;
                list.forEach(item => {
                    if(item.fpxxs){
                        for(let a in item.fpxxs){
                            item[a] = item.fpxxs[a]
                        }
                    }
                })
    ...
    setHeader = async () => {
        ...
        if(getHeader){
            ...
            let len = (getHeader.length-2)*160 + 480;
            let lenX = {x:len}
            getHeader.push({roleId:'operation',name:'操作',fixed:'right',width:90},)
            // this.injections.reduce('updateSingle', 'data.columns',fromJS(getHeader))
            this.injections.reduce('updateObj', {
                'data.columns':fromJS(getHeader),
                'data.tableOption': fromJS(lenX)
            })
        }
        ...
        if (jobList){
            jobList.forEach(item => {
                if(item.roleId !='customerName' && item.roleId != 'helpCode' && item.roleId != 'operation'){
                    jobType.push(item)
                }
            })
