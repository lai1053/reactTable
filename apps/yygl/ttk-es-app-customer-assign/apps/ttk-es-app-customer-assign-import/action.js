...
class action {
    ...
    importAssignData = async () => {
		let params = JSON.parse(sessionStorage.getItem('importData'))
		console.log(params,'params')
		 let res = await this.webapi.assignImport.exportAssignData(params)
		if (res) {
			this.metaAction.toast('success', '下载客户数据成功')
		}
	}
	...
	save = async () => {
		...
		if (file) {
			let path = '/v1/yygl/userCustNew/importNew';
			let res = await this.webapi.assignImport.import(path,file.toJS())
			if(res){
                return res
			}else{
				return false
			...
			/*if (res.state == 0) {
				...
				let rr = await this.webapi.assignImport.getImportAsyncStatus({ sequenceNo: res.orgId }, 5000)
				let result_b = false;
				let e_msg = "导入失败！";
				if (rr) {
					if(rr[0]){
