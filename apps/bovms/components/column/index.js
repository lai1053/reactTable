...
const computeProps = props => {
    ...
    if (!(title && title.type && title.type.name)) title = <Cell>{title || ""}</Cell>
    if (!detailListName) detailListName = "detailList"
    switch (columnType) {
        case "check":
            const allcheck =
                dataSource.length &&
                dataSource.every(o => selectedRowKeys.some(s => s === o[dataIndex]))
            title = (
                <Cell>
                    <Checkbox
                ...
                if (descMark && record[descMark]) {
                    return (
                        <Tooltip
                            ...
                            overlayClassName="check-inv-tool-tip inv-tool-tip-warning">
                            <div style={{ height: "25px" }}>{checkbox}</div>
                        </Tooltip>
                    )
                }
                return checkbox
    ...
    return (
        <Column
            ...
            cell={({ rowIndex, ...props }) => {
                ...
                let lh = noShowDetailList
                    ? 37
                    : (lineHeight || 37) * (rowDetailList.length >= 4 ? 4 : rowDetailList.length)
                if (rowHeightGetter && typeof rowHeightGetter === "function") {
                    lh = rowHeightGetter(rowIndex)
                }
                return (
                    <Cell
                        {...props}
            ...
            footer={() => {
                return <Cell>{footer ? footer() : ""}</Cell>
            }}></Column>
    )
}
/*
