...
const initSelctSubjectOptionList = (props) => {
    return props.defaultItem && props.defaultItem.id && props.defaultItem.codeAndName && props.defaultItem.codeAndName.trim() ? [props.defaultItem] : []
}
/**
 * 资金选择科目
 * metaAction:必须传
...
export default class SelectSubject extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (!isEqual(nextProps.defaultItem, this.state.defaultItem)) {
            ...
            if (nextProps.defaultItem && nextProps.defaultItem.id && !optionList.find(f => f.id === nextProps.defaultItem.id)) {
    ...
    componentDidMount() {
        ...
        //手动点击一次
        if (this.props.autoExpand && (this.props.noShowSelectAssist || !this.getIsCanSelectAssist())) {
            try {
    ...
    getParentCodeId = async () => {
        ...
        if (webapi && webapi.funds) {
            let res = await webapi.funds.getFundParentCodeId({ module: "fund" });
            if (res) {
    ...
    onFocus = async () => {
        ...
        const { webapi, metaAction, isInDropdown } = this.props;
        if (webapi && webapi.funds) {
            let path = isInDropdown ? 'getFlowfundAccountCodeListForAccountMatch' : 'getFundChildAccountCodeList'
            const res = await webapi.funds[path]({ module: 'fund' });
            if (res) {
                // console.log('res', res);
                this.setState({
                    optionList: res
    ...
    getIsCanSelectAssist() {
        const { optionList } = this.state, { assistJSON } = this.props,
            value = this.computePropsValue();
        return JSON.parse(assistJSON || '{}').assistList || (optionList.find(f => f.id == value) || {}).assistList
    }
    openSelectAssist(e) {
    ...
    render() {
        ...
        return (
            <div
                className={!noShowSelectAssist && isCanSelectAssist ? 'bovms-select-subject-container' : ''}
                onBlur={() => {
            ...
            >
                <Select
                    ...
                    value={value}
                    style={{ width: !noShowSelectAssist && isCanSelectAssist ? 'calc(100% - 20px)' : '100%' }}
                    onChange={this.handleChange}
                    // onSelect={this.handleSelect}
                    dropdownMatchSelectWidth={false}
                    dropdownClassName={isCanAdd ? 'bovms-select-subject-dropdown' : 'bovms-select-subject-dropdown no-add'}
                    ...
                    filterOption={(input, option) => {
                        ...
                        return option.props.children && (typeof option.props.children) === 'string' ? option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0 : false;
                    }}
                    ref={this.saveSelect}
                ...
                >
                    {
                        optionList.map(item => (
                            <Option value={item.id} key={item.id} item={item}>{this.getOptionText(item, value)}</Option>
                        ))
                    }
                    {
                        isCanAdd ?
                ...
                </Select>
                {
                    !noShowSelectAssist && isCanSelectAssist ?
                        <a className="assist-btn"
                            unSelectable="on"
                            onClick={::this.openSelectAssist}>辅助</a>
                    : null
    }
            </div>
        );
