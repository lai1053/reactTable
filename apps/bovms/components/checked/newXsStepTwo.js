...
class PopoverFilter extends React.Component {
    constructor(props) {
        ...
        this.filterData = props.metaAction.gf('data.nsqj') || props.metaAction.gf('data.filterData').toJS().yearPeriod;
    }
    ...
    disabledEndDate = current => {
        ...
        // const filterData = metaAction.gf('data.nsqj') || metaAction.gf('data.filterData').toJS().yearPeriod;
        let startDate = moment(this.filterData).startOf('month').subtract(1, 'days').format("YYYY-MM-DD") + ' 23:59:59'
        let endDate = moment(this.filterData).endOf('month').format("YYYY-MM-DD") + ' 23:59:59'
        return current.valueOf() < new Date(startDate).valueOf() || current.valueOf() > new Date(endDate).valueOf()
    }

    render() {
        const { getFieldDecorator } = this.props.form
        let { fpzlDm, rzzt, minAmount, maxAmount, kprq } = this.state
            ...
            <Form className="ttk-filter-form-popover-content">
                ...
                <Form.Item label="开票日期:" {...formItemLayout}>
                    {getFieldDecorator("kprq", {
                        initialValue: kprq
                    })(<RangePicker disabledDate={this.disabledEndDate} defaultPickerValue={moment(this.filterData)} />)}
                </Form.Item>
...
class NewXsStepTwo extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            loading: false,
    ...
    dataFilter(data) {
        let { fpzlDm, rzzt, minAmount, maxAmount, kprq } = this.state.filterParams
        ...
        if (kprq.length) {
            arr = arr.filter(f => moment(f.billDate).isBetween(kprq[0], kprq[1]))
        }
    ...
    onPressEnter = () => {
        ...
        filterData.map(item => {
            if (item.custName.includes(invoiceNub) || item.goodName.includes(invoiceNub) || item.invNo.includes(invoiceNub)) {
                list.push(item)
            } else {
                item.detailList.some(dItem => {
                    if (
                        list.findIndex(f => f.kjxh === item.kjxh) == -1 &&
                        ((dItem.goodsName && dItem.goodsName.includes(invoiceNub)) || (dItem.custName && dItem.custName.includes(invoiceNub)) || (dItem.invNo && dItem.invNo.includes(invoiceNub)))
                    ) {
                        list.push(item)
                    }
                })
    ...
    async purchaseTake() {
        ...
        let res = await this.props.webapi.bovms.querySaleInvoice(this.props.params)
        let { saleList, inCompleteList, onlyPartialDataReturned } = res
        ...
        if (saleList) {
            // 数据处理给每一条数据加一个key
            let accountList = saleList.map((item, index) => {
                item.key = this.generatorRandomNum()
                return item
            })
    ...
    renderFooterAmount = () => {
        ...
        if (invoiceNub || filterParams.fpzlDm || filterParams.maxAmount || filterParams.minAmount || filterParams.kprq.length) {
    ...
    purchaseTakeSave = async () => {
        ...
        //传递参数到父页面请求
        this.props.onSaleTakeSave(accountList, this.toggleLoading)
    }
    ...
    render() {
        const { nsrxz, metaAction } = this.props
        const {
            invoiceNub,
            accountList,
            term,
                    ...
                    >
                        <div className="bovms-app-guidePage-range-step-two-filter">
                            ...
                            <span style={{ float: "right" }}>
                                ...
                                {/* {<Button style={{marginLeft: "8px"}} onClick={this.importData}>导入并替换</Button>} */}
                            </span>
                        </div>
