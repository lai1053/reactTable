...
class SetSameSubject extends React.Component {
            ...
            Object.keys(selectedValues).forEach(key => {
                if (archives[key]) {
                    result.push({
                        ...
                        archivesType: key.indexOf('isexcalc') > -1 ? key.replace('isexcalc', 'exCalc') : `calc${key.substr(0, 1).toLocaleUpperCase()}${key.slice(1)}`,
                    })
                }
            })
    ...
    initKjkm = async () => {
        const { webapi, subjectType, module, metaAction, isStockMonth } = this.props;
        ...
        if (webapi && webapi.bovms && subjectType && module) {
            ...
            const res = await webapi.bovms.getChildAccountCodeList({
                module,
                isStock: module === 'cg' && debitOrCredit === 'debit' && isStockMonth == 1 && subjectType == 'jfkm' ? '0' : undefined,
    ...
    getAllArchive = async () => {
        const res = await this.webapi.bovms.allArchive({ isEnable: true })
        if (res) {
            ...
            res['自定义档案'] && res['自定义档案'].forEach(item => {
                labelOption[item.calcName.toLocaleLowerCase()] = item.name;
                archives[item.calcName.toLocaleLowerCase()] = item.userDefineArchiveDataList || [];
            });
    ...
    archiveFocus = async (archiveName) => {
        ...
        if (archiveName.indexOf('exCalc') > -1) {
            ...
            response = await this.webapi.bovms.userDefineItem(parmasObj)
        } else {
            response = await this.webapi.bovms.fixedArchive(params, archiveName)
        }
        if (response) {
    ...
    handleSubjectChange = (value, option) => {
        ...
        // console.log('forItems', formItems)
