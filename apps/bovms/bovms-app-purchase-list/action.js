...
class action {
    ...
    onInit = async ({ component, injections }) => {
        ...
        // console.time('业务进项－页面初始化请求接口耗时')
        await this.getIsCollectInv()
        // console.timeEnd('业务进项－页面初始化请求接口耗时')
        // 再次进入 refresh
    ...
    async getTableSettingData() {
        const res = await this.webapi.bovms.queryColumnSetup({ module: 2 })
    ...
    getIsCollectInv = async nsqj => {
        ...
        const resStock = await resStockFun,
            ...
            resTable = await this.webapi.bovms.queryPurchasePageList({
                entity: {
    ...
    initPage = async (current, pageSize) => {
        ...
        const webRes = await this.webapi.bovms.queryPurchasePageList({
            entity: {
    ...
    delVoucher = async () => {
        ...
        if (confirm) {
            ...
            const res = await this.webapi.bovms.deletePurchaseVoucher({
    ...
    delInvoice = async () => {
        ...
        if (confirm) {
            ...
            const res = await this.webapi.bovms.deleteBillPurchase({
