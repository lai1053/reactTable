...
class action {
	...
	load = async () => {
		...
		if (this.component.props.personId || this.component.props.personId === 0) {
			response = await this.webapi.unit.query(this.component.props.personId)
		} else {
			code = await this.webapi.unit.getCode()
		}
	...
	save = async () => {
		...
		if (this.component.props.personId || this.component.props.personId === 0) {
			...
			response = await this.webapi.unit.update(form)
		} else {
			response = await this.webapi.unit.create(form)
		}
		this.clickStatus = false
