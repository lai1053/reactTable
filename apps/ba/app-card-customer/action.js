...
class action {
	...
	load = async () => {
		...
		category =  await this.webapi.customer.cats(requestCats)
		if(category) data.category = category;
		account = await this.webapi.customer.account()
		if (code){ data.code = code, data.formName = this.component.props.formName }
	...
	save = async () => {
		...
		if(!form.receivableAccountId && !(this.component.props.personId || this.component.props.personId === 0)){
			let subjectCreate = await this.webapi.customer.canGenerateAccount({name: form.name.trim()})
			if(subjectCreate.length > 0 && this.noPointStatus == 1){
				...
				if(noPointSelectStatus){
					this.webapi.customer.updateBatchByparamKey([{paramKey: ["PromptGenerateSpecificAccount4Customer"],paramValue: "0"}])
				}
			}
		}
		let response
	...
	selectCats = value => {
		this.metaAction.sf('data.form.categoryId',value)
	}
	changeCheck = (str) => {
		const form = this.metaAction.gf('data.form').toJS()
		switch (str){
	...
	check = async (option) => {
		...
		if (option.path == 'data.form.code') {
			...
			return {errorPath: 'data.other.error.taxNumber', message: option.value ? option.value && (option.value.length == 20 || option.value.length == 15 || option.value.length == 18) ? '' : '税号应为15，18或20位' : ''}
		}
	...
	managerItem = async (str) => {
		...
		cats =  await this.webapi.customer.cats(requestCats)
	...
	payableAccountChange = (name) => {
		let data = this.metaAction.gf('data').toJS(),
			changeItem = { noChild:false, id: data.form.receivableAccountId ? data.form.receivableAccountId: '' },
			saveItem = {
				receivableAccount:[],
		...
		if(data.other.glAccounts){
			for(let accountIndex = 1; accountIndex < data.other.glAccounts.length; accountIndex++){
				...
				if (Item.name == name) {
					...
					}else if(Item.code.includes(2202)){
			...
			if(saveItem.receivableAccount.length){
			...
			}else if(saveItem.payableAccount.length){
