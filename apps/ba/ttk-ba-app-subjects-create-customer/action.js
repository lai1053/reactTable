...
class action {
    ...
    load = async () => {
        ...
        let gl1 = this.webapi.gl({"codeGrade1": 1122}),
            gl2 = this.webapi.gl({"codeGrade1": 1221}),
            gl3 = this.webapi.gl({"codeGrade1": 2203}),
        ...
        let option = {
                ...
                other: {
                    ...
                    1122:(await gl1).glAccounts,
                    1221:(await gl2).glAccounts,
                    2203:(await gl3).glAccounts,
                }
            }
    ...
    onOk = async () => {
        ...
        if (!specification.length) {
            this.metaAction.toast('warning', '应收账款取值规则，不允许为空');
            return false;
        }
        ...
        endArr.forEach(function (endNode) {
            let obj = {
                name: endNode.name,
                receivableAccountId: endNode.id,
        ...
        if(res){
            if(res.length == 0){
                ...
                await this.metaAction.modal('success', {
                    ...
                    content: <div>
                        <div>失败原因：</div>
                        <div>存在名称相同的客户档案，{str}无法生成客户档案，请修改后重新生成</div>
                    </div>,
                    okText: '确定'
                })
            }
    ...
    subjectChange = (checkedList) => {
        ...
        if(checkedList.length == 0)  this.metaAction.toast('warning', '应收账款取值规则，不允许为空')
