...
class reducer {
	...
	load = (state, value, enumList, queryAll) => {
		if (value && value.list) {
		...
		if(enumList){  
			state = this.metaReducer.sf(state, 'data.unitList', fromJS(enumList.unitList)) 
		}
		if(queryAll){
			queryAll.unshift({id: 0, name: '全部'})
			state = this.metaReducer.sf(state, 'data.goodsTypes', fromJS(queryAll)) 
		}
		return state 
	} 
