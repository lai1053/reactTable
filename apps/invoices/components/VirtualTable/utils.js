...
function getSelectCol(props, flatCols) {
    ...
    const { selectedRowKeys, type, columnWidth, selections, onChange, getCheckboxProps } =
        rowSelection || {}
    let _columns = []
    if (selectedRowKeys && onChange) {
        const render = (text, record, index) => {
            const dataIndex = typeof rowKey === "function" ? rowKey(record) : rowKey,
                key = record[dataIndex],
                rowSelected = selectedRowKeys.some(s => String(s) === String(key)),
                disabled =
                ...
                handelChange = e => {
                    ...
                    onChange(Array.from(new Set(keys)), record, e.target.checked)
                }
            if (type === "radio") {
                return (
                    <Radio
        ...
        const dropdownMenuItems = selections
            ...
            : null
        const dropdownMenu = selections ? <Menu>{dropdownMenuItems}</Menu> : null
        ...
        const handelAllChange = e => {
            if (e.target.checked) {
                onChange(dataSource.map(d => d[typeof rowKey === "function" ? rowKey(d) : rowKey]))
            } else {
                onChange([])
            }
        }
        const title = (
            <div className="select-cell-header">
                <Checkbox
                    ...
                    onChange={handelAllChange}></Checkbox>
                {dropdown}
            </div>
        )
        const selectCol = {
...
var getEstimatedTotalSize = function (props, instanceProps) {
    ...
    var needReCalculate = instanceProps.needReCalculate
    ...
    if (
        ...
        instanceProps.dataSourceIsEqual = true
        instanceProps.needReCalculate = false
        instanceProps.itemMetadataMap = {}
        instanceProps.lastMeasuredIndex = -1
        itemMetadata = getItemMetadata(props, itemCount - 1, instanceProps, false, needReCalculate)
        needReCalculate && (instanceProps.lastMeasuredIndex = lastMeasuredIndex)
    }
    return itemMetadata
        ? itemMetadata.offset + itemMetadata.size + (itemMetadata.expandedHeight || 0)
        : 0
}
...
var getItemMetadata = function (
    ...
    if (!isReCalc && item) {
        return item
    } else if (index > lastMeasuredIndex) {
        var offset = 0

        if (lastMeasuredIndex >= 0) {
            var itemMetadata = itemMetadataMap[lastMeasuredIndex]
            itemMetadata && (offset = itemMetadata.offset + itemMetadata.size)
        }
...
var findNearestItem = function (props, instanceProps, offset) {
    var itemMetadataMap = instanceProps.itemMetadataMap,
        lastMeasuredIndex = instanceProps.lastMeasuredIndex,
        rowCount = Array.isArray(props.dataSource) ? props.dataSource.length - 1 : 0
    if (lastMeasuredIndex > rowCount) {
        lastMeasuredIndex = rowCount
        instanceProps.lastMeasuredIndex = rowCount
    }
...
var findNearestItemBinarySearch = function (props, instanceProps, high, low, offset) {
    while (low <= high) {
...
var findNearestItemExponentialSearch = function (props, instanceProps, index, offset) {
...
const renderList = props => {
    ...
    const record = dataSource[index],
        ...
        dataIndex = typeof rowKey === "function" ? rowKey(record) : rowKey || "key",
        rowSelected = selectedRowKeys.some(s => String(s) === String(record[dataIndex])),
    ...
    if (itemMetadata) {
        ...
        if (itemMetadata.expanded) {
            ...
            expandedRowStyle.top = itemMetadata.offset + itemMetadata.expandedHeight
            expandedRowStyle.left = 0
            expandedRow = (
                <div
                    className={classNames(`${prefixCls}-body-row`, "row-expanded")}
    ...
    return (
        <React.Fragment>
            <RowComponent
                ...
                onDoubleClick={e => onRowEvent(props, rowIndex, "onDoubleClick", e)}
                onContextMenu={e => onRowEvent(props, rowIndex, "onContextMenu", e)}>
                {renderCol(
                    columns,
                    record,
                    rowIndex,
...
const renderCol = (
    ...
    const expandedButtonColIndex = cells.findIndex(f => f.key === "select-col-key") + 1
    ...
    return cells.map((item, i) => {
        ...
        const dataIndex =
            typeof item.dataIndex === "function" ? item.dataIndex(record) : item.dataIndex
        ...
        let colWidth = 0,
            colFlex = 0,
            { props, children } =
                Object.prototype.toString.call(cell) === "[object Object]" ? cell : {}
        if (props) {
            const { colSpan } = props
        ...
        return (
            <CellComponent
                ...
                className={classNames(
                    ...
                    {
                        "fixed-left-last": window.cssSupportSticky && fixed === "left" && lastFixed,
                    },
                    {
                        "fixed-right-last":
                            window.cssSupportSticky && fixed === "right" && lastFixed,
                    }
                )}
                style={style}
...
function onRowMouseEnter(props, rowIndex, e) {
    let rows = document.querySelectorAll("." + props.tableId + " .row-" + rowIndex)
    if (rows && rows.length > 0) {
        let dom
        for (var i = 0; i < rows.length; i++) {
            dom = rows[i]
            if (dom) {
                dom.className = [...new Set(dom.className.split(" ").concat("row-hover"))].join(" ")
            }
        }
    }
...
function onRowEvent(props, rowIndex, eventName, e) {
    const { onRow, dataSource = [], startIndex } = props,
        onEvent = ((onRow && onRow(dataSource[rowIndex - startIndex])) || {})[eventName]
    onEvent && onEvent(e)
}
