...
class action {
    ...
    load = async (kjxh) => {
        ...
        let taxRates = []
        if(this.component.props.justShow != true)taxRates = await this.webapi.invoice.getSlvcsList({nsrxz, fplx: 'jxfp', fpzlDm: '17'})
        ...
        if (this.isV2Component() && this.component.props.justShow != true) {
            sbytcsRes = await this.webapi.invoice.getSbytcsList(`nsrxz=YBNSRZZS&fplx=jxfp&fpzlDm=17`)
        }
        if (!this.mounted) return //组件已经卸载
    ...
    onOk = async () => {
        if(this.component.props.justShow){
            // 什么都不做 直接关闭页面
        }else {
    ...
    handleGetPopupContainer = () => {
        ...
        // return document.getElementById('inv-app-pu-toll-invoice-card')
    }
    ...
    // 名称改变
    hwmcChange = (rowIndex,rowData,v) =>{
        let data = [
            {
                hwmc:'高速公路通行服务',
                spbm:3040502020301000000,
                svl:0.09
            },
            {
                hwmc:'江河等闸口通行服务',
                spbm:3040502020302000000,
                svl:0.09
            },{
                hwmc:'桥梁通行服务',
                spbm:3040502020303000000,
                svl:0.09
            },{
                hwmc:'其他道路通行服务',
                spbm:3040502020399000000,
                svl:0.09
            }
        ]
        data.forEach((item)=>{
            if(v === item.spbm) v= item
        })
        let obj;
        if (v instanceof Object) {
            let {hwmc, spbm,slv } = v;
    ...
    renderHwmcSelectOption = ()=> {
        ...
        return data.map((d, index) => <Option title={d.hwmc} key={d.spbm} value={d.spbm} style={{ 'font-size': '12px', 'height': '36px', 'line-height': '26px' }}>{d.hwmc}</Option>)
    }
