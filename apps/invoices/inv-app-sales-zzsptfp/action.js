...
class action {
    ...
    load = async (kjxh,redDate) => {
        ...
        if (kjxh) {
            ...
            let initInfo = await this.webapi.invoices.initiateAddOfXxfp({ qyId, nsrsbh,fpzlDm:'04',skssq:this.component.props.nsqj})
            if (initInfo) {
                initData = {
                    details: [{}, {}, {}, {}],
                    xfmc: initInfo.xfmc,
        ...
        let details = initData.details && initData.details.map(item => {
            return {
                ...
                slList: fromJS(slRes || []),
                jsfsList: fromJS(editInitData.noZero ? jsfsRes.filter(f => f.jsfsDm !== '2' && f.jsfsDm !== '3') : jsfsRes),
    ...
    onOk = async () => {
        if(this.component.props.justShow === true){
            ...
            // params.skssq = moment(periodDate).subtract(1, 'month').format('YYYYMM')
    ...
    jzjtbzChange = (value) => {
        ...
        details.map(item => {
            if (item.jsfsList) {
                // N:有免 ,Y:无免
                let jsfsListCache = value == 'N' ? this.metaAction.gf('data.jsfsList').toJS() : item.jsfsList.filter(f => f.jsfsDm !== '2' && f.jsfsDm !== '3')
                item.jsfsList = jsfsListCache
                if (item.jsfsDm !== undefined && !jsfsListCache.find(f => f.jsfsDm == item.jsfsDm)) {
                    item.jsfsDm = undefined
                }
            }
            return item
        })
    ...
    onCellChange = (index, field, value) => {
        ...
        switch (field) {
            ...
            case 'hwmc': //服务名称
                if (item.hwmc !== value) {
                    if (typeof value == 'object') {
                        let _slList = this.metaAction.gf('data.slList').toJS(),
                            _hwlxDm = this.calcHwlx(value.spbm),
                            jsfsDm = this.calcJsfs(value.slv),
                            _jsfsDm = item.jsfsList.find(f => f.jsfsDm == jsfsDm) ? jsfsDm : undefined,
                            _slv = _slList.find(f => f.slv == value.slv) ? value.slv : undefined;
                        details[index].jsfsDm = _jsfsDm
                        json = {
                            ...
                            [`data.form.details.${index}.jsfsDm`]: _jsfsDm,
                        ...
                        this.taxChangeJzjtbz(details, json)
                    } else {
                        json = {
            ...
            case 'slv': //税率
                if (item.slv !== Number(value)) {
                    let jsfsDm = this.slChangeJsfs(value),
                        _jsfsDm = item.jsfsList && item.jsfsList.find(f => f.jsfsDm === jsfsDm) ? jsfsDm : undefined
                    details[index].jsfsDm = _jsfsDm
                    json = {
            ...
            case 'jsfsDm': // 计税方式
                if (item.jsfsDm !== value) {
                    ...
                    if (fplyLx == 2) {
                        // 2：录入
                        let { slList, slv } = this.jsfsChangeSl(value, taxRate)
                        let _slv = slList.toJS().find(f => f.slv == slv) ? slv : undefined
                        json[`data.form.details.${index}.slList`] = slList
        ...
        if (json) {
            ...
            // console.log(json, this.metaAction.gf('data').toJS())
        }
    }
    taxChangeJzjtbz = (details, json) => {
        if (details.some(f => f.jsfsDm == '2' || f.jsfsDm == '3')) {
            // 计税方式选择“免抵退”、“免税”的，即征即退为“否”，置灰，不可修改
            json['data.other.jzjtbzDisable'] = true
            json['data.form.jzjtbz'] = 'N'
        } else {
            json['data.other.jzjtbzDisable'] = false
        }
    }
    inputBlur = (index, field) => {
        index = parseInt(index)
        let json = null,
            details = this.metaAction.gf('data.form.details').toJS(),
            item = details ? details[index] : {},
        ...
        switch (field) {
            case 'sl': //数量
                ...
                json = {
                    // [`data.form.details.${index}.quantity`]: value,
            ...
            case 'je': //金额
                ...
                json = {
                    ...
                    [`data.form.details.${index}.dj`]: temp !== undefined ? parseFloat(temp).toFixed(4) : undefined,
                    [`data.form.details.${index}.se`]: !isNaN(amount) && taxRate > -1 ? this.calcSe(amount, taxRate) : undefined,
                }
                break
            case 'se': // 税额
                if (!isNaN(taxAmount)) {
        ...
        switch (jsfs) {
            ...
            case undefined:
                ...
                if (nsrxz == 'YBNSRZZS') {
                    ...
                    slListNew = slList.filter(f => f.slv == 0.05 || f.slv == 0.03 || f.slv == 0 || f.slv == 0.01)
                }
                break
        }
        return { slList: fromJS(slListNew), slv }
    ...
    checkForm = () => {
        ...
        if (!form.gfsbh) {
            ...
            flag = false
        } else if (form.gfsbh.trim().length < 15) {
            error.gfsbh = '购买方纳税人识别号不能小于15个字符'
            flag = false
        }
            ...
            form.details.forEach((item, index) => {
                ...
                if (item.key) {
                    ...
                    if (item.jzjtDm === undefined) {
                        // error.details[index].jzjtDm = true
                        // flag = false
                        item.jzjtDm = 'N'
                    }
                }
