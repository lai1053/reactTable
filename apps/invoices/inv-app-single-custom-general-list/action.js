...
import { generalBtnType } from './fixedData'
...
class action {
    ...
    getColumns = async () => {
        ...
        const pageID = 'singleGeneralCustom'
        const resp = await this.webapi.invoice.queryColumnVo({
            pageID
        })
        if (!this.mounted) return
        const columns = this.metaAction.gf('data.columnData2').toJS()
        if (resp) {
    ...
    load = async () => {
        const currentOrg = this.metaAction.context.get('currentOrg')
        const qyId = currentOrg && currentOrg.id
        const nsrsbh = currentOrg && currentOrg.vatTaxpayerNum
    ...
    loadSaleInvoice = async type => {
        ...
        let nsqj =
            this.metaAction.gf('data.nsqj') &&
            this.metaAction.gf('data.nsqj').format('YYYY-MM-DD')
        nsqj = moment(nsqj)
            .subtract(1, 'month')
            .format('YYYYMM')
            .substr(0, 6)
    ...
    renderStatus = (record, type) => {
        ...
        overlayClassName={
          status === 0 ? 'inv-tool-tip-normal' : 'inv-tool-tip-warning'
        }
        title={status === 0 ? `采集时间：${time}` : message}>
        <Icon
          type={status === 1 ? 'exclamation-circle' : 'check-circle'}
          className={
            status === 1 ? 'inv-custom-warning-text' : 'inv-custom-success-text'
          }
        />
      </Tooltip>
        ) : null
    ...
    renderTaxAmount = (text, record, row) => {
        if (row.isToolTip) {
            let message = record[`${row.type}SeMsg`] ?
                record[`${row.type}SeMsg`].split(';') :
                []
          ...
          <span className="text-tax-amount">{text}</span>
        </Tooltip>
            ) : (
                <span className="text-tax-amount">{text}</span>
            )
        ...
        if (row.fieldName === 'totalAmount' && record.limitRate) {
          ...
          <Tooltip
            ...
            <Icon
              ...
              className="inv-custom-warning-text"
            />
          </Tooltip>
        ...
        column.forEach((item, index) => {
            if (item.isVisible) {
                ...
                if (item.children) {
                    ...
                    item.children.forEach(subItem => {
                        if (subItem.isSubTitle) {
                            ...
                            if (
                                !(item.fieldName === 'jxfpyrz' && column[index + 1].isVisible)
                            ) {
                                col = {
                                    title: subItem.caption,
                                    dataIndex: subItem.fieldName,
                ...
                <div
                  className={
                    showTableSetting
                      ? 'setting-box setting-active'
                      : 'setting-box setting'
                  }
                  onClick={() => this.showTableSetting({ value: true })}>
                  <Icon type="setting" name="setting" />
                  {showTableSetting ? (
                    <span style={{ paddingLeft: '5px', display: 'inline' }}>
                      列设置
                    </span>
                  ) : null}
                </div>
    ...
    salesPurchaseClick = (record, t) => {
        ...
        if (this.component.props.setPortalContent) {
            ...
            let appName = `${
        t == 1 ? 'inv-app-batch-sale-list' : 'inv-app-batch-purchase-list'
      }?swVatTaxpayer=0&nsqj=${nsqj}&userDetail=${userDetail}&qyId=${
        record.qyId
      }&xgmJbOrYb=${record.cwbbType === '季报' ? '1' : '0'}`
    ...
    getTableScroll = e => {
        try {
            ...
            let appDom = document.getElementsByClassName(
                'inv-app-single-custom-general-list'
            )[0] //以app为检索范围
