...
class Download extends React.Component {
  ...
  handelSubmit = async obj => {
    ...
    const res = await new Promise((resolve, reject) => {
      ...
      setTimeout(async () => {
        const response = await this.props.webapi.person.downInvoice(
          requestOption
        );
        if (response) {
          this.props.lodingF();
  ...
  render() {
    ...
    return (
      <div style={{ height: 85 }}>
        ...
        <div
          style={{ textAlign: "center", fontSize: "12PX", marginLeft: "-30px" }}
        >
          <span style={{ color: "orange" }}>温馨提示：</span>
          <span>包含本月抵扣发票和本月开具发票</span>
        </div>
      </div>
    );
  }
