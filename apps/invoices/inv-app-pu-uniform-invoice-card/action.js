...
class action {
    ...
    load = async (kjxh) => {
        ...
        let taxRates = []
        if(this.component.props.justShow != true)taxRates = await this.webapi.invoice.getSlvcsList({nsrxz, fplx: 'jxfp', fpzlDm: '14'})
        ...
        if (this.isV2Component() && this.component.props.justShow != true) {
            sbytcsRes = await this.webapi.invoice.getSbytcsList(`nsrxz=YBNSRZZS&fplx=jxfp&fpzlDm=14`)
        }
    ...
    checkForm = () => {
        ...
        if (!this.isV2Component() && form.dkyf === undefined) {
            error.dkyf = '抵扣月份不能为空'
            flag = false
        }
    ...
    amountChange = (rowIndex, rowData, v) => {
        if (!Number(v)) {
            ...
            if (this.isV2Component() && !this.notAllowEditNf06()) {
                this.injections.reduce('updateSfs', {
                    'data.form.nf06': this.sumColumn('je').replace(/,/g, '') * 0.09,
                    'data.error.nf06': '',
                })
            }
            return
    ...
    formDataV2 = (form) => {
        ...
        if (form.bdzt && form.fplyLx == 2) {
            // console.log('formDataV2:', form.nf06, form.nf06 != null && form.nf06 != '')
            form.bdlyLx = form.bdlyLx || '1';
            form.dkyf = form.bdlyLx == 1 ? (form.dkyf || this.component.props.nsqj) : null;
            form.nf06 = form.nf06 !== null && form.nf06 !== '' ? form.nf06 : form.hjje && parseFloat((form.hjje * 0.09)).toFixed(2);
    ...
    handleFormNumberBlur = (path = 'nf06') => (e) => {
        ...
        let hjse = Number(this.sumColumn("je").replace(/,/g, ''));
        hjse = parseFloat(hjse * 0.09).toFixed(2);
        if (!hjse) {
            hjse = 0;
        }
        if (v === '' || isNaN(Number(v))) {
    ...
    handleBdlylxChangeV = (v) => {
        ...
        this.injections.reduce('updateSfs', {
            ...
            'data.error.dkyf': '',
            'data.error.bdlyLx': '',
            'data.form.nf06': nf06 || nf06 === 0 ? nf06 : this.sumColumn('je').replace(/,/g, '') * 0.09,
            'data.error.nf06': '',
        })
    }
    // 陕西试点2.0版本
