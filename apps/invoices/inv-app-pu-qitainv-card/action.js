...
class action {
    ...
    load = async (kjxh,redDate) => {
        ...
        if (kjxh) {
            ...
            let gfinfo = await this.webapi.invoice.initiateAddOfJxfp({ nsrsbh, qyId ,fpzlDm:'99',skssq});
            if (!this.mounted) return
            if (gfinfo) {
                initData = {
                    ...initData,
                    ...
                    fplyLx: '2',
                    bdlyLx: '', // 1-抵扣， 2-退税， 3-代办退税， 4-不抵扣自
        ...
        let taxRates = []
        if(this.component.props.justShow != true)taxRates = await this.webapi.invoice.getSlvcsList({nsrxz, fplx: 'jxfp', fpzlDm: '99'})
        if (!this.mounted) return
        ...
        form.mxDetailList.forEach((t)=>{
            ...
            if(t.hsje === null)  return  t.hsje = undefined
        ...
        if(this.component.props.justShow === true){
            ...
            if (this.component.props.kjxh && res === null) {
                ...
                let initFormData = {
                    ...
                    bdzt: this.component.props.fromModule === 'InvoiceAuthentication' ? 0 : 0, //认证状态 发票采集默认选中 其它模块默认不选中
                    dkyf: null, // 默认为当前属期
        ...
        if (!mxDetailList.length) {
            ...
            mxDetailList.forEach((item, index) => {
                ...
                if (item.hsje === undefined) {
                    error.mxDetailList[index].hsje = true;
                    flag = false
                }
    ...
    //含税金额改变
    hsAmountChange = (rowIndex, rowData, v) => {
        if (!Number(v)) {
            let obj = {
                [`data.form.mxDetailList.${rowIndex}.hsje`]: v,
        ...
        // 税额＝含税金额 - 不含税金额，保留2位小数
        //如果税率为空不含税金额=含税金额，税额为空
        // 如果数量为0 ，单价为0
        // 如果数量不为0，单价＝金额÷数量
        let taxRate = rowData.slv || 0
        
        let amount = Number(v), //含税金额
            je = amount / (1+taxRate), //不含税金额 = 含税金额/（1+税率），保留2位小数
            quantity = utils.number.round(rowData.sl, 6), //数量
            tax = amount-je     //税额=含税金额 - 不含税金额
        const price = quantity ? utils.number.round(je / quantity, 4) : 0;
        let obj = {
            [`data.form.mxDetailList.${rowIndex}.hsje`]: v === '' ? undefined : amount,
            [`data.form.mxDetailList.${rowIndex}.je`]: v === '' ? undefined : je.toFixed(2),
            [`data.form.mxDetailList.${rowIndex}.se`]: v === '' || rowData.slv === undefined ? undefined : tax.toFixed(2),
    ...
    taxRateChange = (rowIndex, rowData, v) => {
        ...
        let taxRate = v || 0; //税率
        let amount = utils.number.round(rowData.hsje, 2); //含税金额
        let  je = amount / (1+taxRate).toFixed(2) //不含税金额 = 含税金额/（1+税率），保留2位小数
        let tax = amount - je
        let obj = {
            [`data.form.mxDetailList.${rowIndex}.slv`]: v,
            [`data.form.mxDetailList.${rowIndex}.se`]: v === undefined ? undefined : tax.toFixed(2),
            [`data.form.mxDetailList.${rowIndex}.je`]: v === undefined ? undefined : je.toFixed(2),
    ...
    moneyToBig = (t) => {
        ...
        let a = this.amountTotal(true),
            ...
            c = Number(a) + Number(b)
        if (t) {
            hj.jshjDx = c && this.moneySmalltoBig(c.toFixed(2)) || undefined
            return c && this.moneySmalltoBig(c.toFixed(2)) || undefined
        }
        hj.jshj = c && parseFloat(c).toFixed(2) || undefined
    ...
    disabledDateQ = (currentDate) => {
        ...
        if (skssq && currentDate) {
            ...
            /*  if (this.component.props.fromModule === 'InvoiceAuthentication') {
                  ...
                  let startDate = moment(this.component.props.nsqj + '99', 'YYYY-MM-DD').subtract(360, 'days');
