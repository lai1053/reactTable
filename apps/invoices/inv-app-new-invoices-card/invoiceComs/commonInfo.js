        ...
        const dom = data.map((e, i) => (
            <div className="invoice-common-info-row">
                {e.map((se, si) => {
                    ...
                    html.push(
                        <div
                            className="invoice-common-info-row-label"
                            style={se.labelWidth ? { width: se.labelWidth } : { flex: 1 }}>
                    ...
                    switch (se.type) {
                        case "select":
                            control = readOnly ? (
                                <div className="invoice-info-static-cell">{se.value}</div>
                            ) : (
                        ...
                        case "input":
                            ...
                            control = readOnly ? (
                                ...
                                <Tooltip
                                    ...
                                    visible={
                                        se.errMsg && se.errMsg.indexOf("不可为空") == -1
                                            ? true
                                            : false
                                    }
                                    ...
                                    <Form.Item
                                        key={se.key}
                                        validateStatus={se.errMsg ? "error" : null}>
                                        {getFieldDecorator(
                                            `${se.key}`,
                                            formItemOptions
                                        )(<Input {...se}></Input>)}
                            ...
                            control = (
                                <Form.Item key={se.key}>
                                    {getFieldDecorator(
                                        ...
                                        <Checkbox style={{ marginLeft: "8px" }} {...se}>
                                            {se.checkBoxLabel}
                                        </Checkbox>
                                    )}
                                </Form.Item>
                    ...
                    html.push(
                        <div
                            className="invoice-common-info-row-control"
                            style={se.controlWidth ? { width: se.controlWidth } : { flex: 1 }}>
                            {control}
                        </div>
                    )
                    return html
    ...
    render() {
        ...
        return (
            <Form autocomplete="off">
                <div className={`invoice-common-info ${className}`}>{this.getDom()}</div>
            </Form>
        )
    }
}
