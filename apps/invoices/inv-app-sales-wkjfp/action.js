...
class action {
    ...
    load = async (kjxh,redDate) => {
        ...
        if (kjxh) {
            ...
            if (res) {
                ...
                res.mxDetailList.forEach((item,index)=>{
                    item.key = index+1
                })
            ...
            let initInfo = await this.webapi.invoices.initiateAddOfXxfp({ qyId, nsrsbh,fpzlDm:this.component.props.fpzlDm,skssq:this.component.props.nsqj })
            if (initInfo) {
                initData = {
                    details: [{}, {}, {}, {},{}, {},{}, {}],
                    xfmc: initInfo.xfmc,
        ...
        if(this.component.props.justShow === true){
            ...
            if (kjxh && res === null) {
                ...
                this.injections.reduce('setState', 'data.form', fromJS({
                    ...
                    details: [blackDetail, blackDetail, blackDetail, blackDetail,blackDetail,blackDetail,blackDetail,blackDetail]
                }))
            }
            return false
        }
    ...
    onCellChange = (index, field, value) => {
        ...
        let json = null,
            ...
            price = item.dj, //单价
            hsAmount =  Number(item.hsje), // 含税金额
        ...
        switch (field) {
            ...
            case 'sl': //数量
                ...
                json = {
                    ...
                    [`data.form.details.${index}.dj`]: !Number(amount/val) ? undefined : (amount/val)
                }
                break
            ...
            case 'price': //单价
                break
            case 'hsje': // 含税金额
                hsAmount =Number(value)
               if(isNaN(taxRate))taxRate=0
                let je = hsAmount/(1+taxRate)
                let se = hsAmount - je
                json = {
                    [`data.form.details.${index}.hsje`]: value,
                    [`data.form.details.${index}.je`]: !isNaN(je) ? parseFloat(je).toFixed(2) : undefined,
                    [`data.form.details.${index}.se`]: !isNaN(se) ? parseFloat(se).toFixed(2) : undefined,
                    }
                break
            case 'je': //金额
                break
            ...
            case 'hwmc': //服务名称
                if (item.hwmc !== value) {
                    if (typeof value == 'object') {
                        ...
                        let  _slv = _slList.find(f => f.slv == value.slv) ? value.slv : undefined;
                        json = {
            ...
            case 'slv': //税率
                if (item.slv !== Number(value)) {
                    let je = hsAmount/(1+value)
                    let se = hsAmount - je
                    json = {
                        [`data.form.details.${index}.slv`]: value > -1 ? Number(value) : undefined,
                        [`data.form.details.${index}.hsje`]: !isNaN(hsAmount) ? parseFloat(hsAmount).toFixed(2) : undefined,
                        [`data.form.details.${index}.je`]: !isNaN(je) ? parseFloat(je).toFixed(2) : undefined,
                        [`data.form.details.${index}.se`]: !isNaN(se) ? parseFloat(se).toFixed(2) : undefined,
            ...
            case 'jsfsDm': // 计税方式
                if (item.jsfsDm !== value) {
                    ...
                    if (fplyLx == 2) {
                        ...
                        //console.log(temp.toJS(), taxRate,value);
    ...
    hsAmountTotal = (t = true) => {
        let fplyLx = this.metaAction.gf('data.form.fplyLx');
        let data = this.metaAction.gf('data.form').toJS();
        if (fplyLx === '1') {
            return `${isNaN(data.hsje)?'':data.hsje}`
        }
        return this.calcTotal('heje', t)
    }
    ...
    checkForm = () => {
        let form = this.metaAction.gf('data.form').toJS(),
            ...
            obj = {};
        error.details = []
        if(this.component.props.fpzlDm === '05'){
        ...
        if(this.component.props.fpzlDm === '09'){
            if (!form.fphm) {
                error.fphm = '发票号码不能为空'
                flag = false
            } else if (form.fphm) {
