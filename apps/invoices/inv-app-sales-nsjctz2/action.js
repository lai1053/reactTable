...
class action {
    ...
    load = async (kjxh) => {
        ...
        if (kjxh) {
            ...
            let initInfo = await this.webapi.invoices.initiateAddOfXxfp({ qyId, nsrsbh,fpzlDm:'08',skssq:this.component.props.nsqj })
            if (initInfo) {
                initData = {
                    details: [{}, {}, {}, {}],
                    xfmc: initInfo.xfmc,
        ...
        jsfsRes = jsfsRes.filter(f=> f.jsfsMc === "一般计税")
        ...
        switch (jsfs) {
            ...
            case "1":
                ...
                if (nsrxz == 'YBNSRZZS') {
                    ...
                    // 5%、3%
                    slListNew = slList.filter(f => f.slv == 0.05 || f.slv == 0.03)
                }
                break
            case "2":
                //免抵退
