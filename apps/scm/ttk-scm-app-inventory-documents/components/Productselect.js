...
class Productselect extends React.Component {
    constructor(props){
        ...
        this.state={
            list: [],
            tableOption: props.type == 'materialPro' ? {x: 1400} : {},
            loading: true,
            inventoryType: '',
            searchContent: '',
            tableCheckbox: {
            ...
            amount: props.amountyu, //产成品入库直接材料合计
            ...
            productionAccounting: props.proMethod,
            syAmount: props.amountyu, //剩余金额
        }
    ...
    onOk = () => {
        ...
        // 把选择原料的数量必填去掉
        if (quantityEmp && type != 'materialPro') {
        ...
        newSelectList = newSelectList.map((item, index) => {
            ...
            if(type == 'materialPro' && typeof item.periodEndAmount == 'undefined') item.periodEndAmount = null
            if (type == 'materialPro') item.price = item.periodEndPrice
    ...
    handleQuantityBlur = (v, indexList) => {
        let list = this.state.list, type = this.props.type
    ...
    automaticAcquisition = () => {
        ...
        console.log(lastIndex, amountTotal, pickingAmount, amount, 'lastIndex')
    ...
    handleBlur = (e, type) => {
        ...
        // this.setState({
        //     [type]: isOk ? addThousPos(value, true, null, 2) : ''
        // })
    }
    ...
    render() {
        const {list, tableCheckbox,  tableOption, loading, sort, amount, isVisiablePer,
             productAmount, pickingAmount, percentage, productionAccounting, syAmount} = this.state

        let {title, selectList, type} = this.props
            ...
            <div>
                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                    ...
                    <div className='butDiv'>
                        <Form style={type == 'materialPro' ? {display: 'visiable'}:{display: 'none'}}>
                            <Form.Item label='领料金额'><Input.Number onBlur={(e) => this.handleBlur(e, 'pickingAmount')} value={pickingAmount}/></Form.Item>
                        </Form>
                        <Button onClick={() => this.automaticAcquisition()} style={type == 'materialPro' ? {display: 'visiable'}:{display: 'none'}}>自动领料</Button>
                        <Button onClick={() => this.fullAcquisition()} style={type == 'materialPro' ? {display: 'visiable'}:{display: 'none'}}>全额领料</Button>
                        <Button onClick={() => this.addInventory()}>新增存货</Button>
                    </div>
                </div>
                <div className='middleProduction' style={type == 'materialPro' && productionAccounting != 0 ? {display: 'visiable'}:{display: 'none'}}>
