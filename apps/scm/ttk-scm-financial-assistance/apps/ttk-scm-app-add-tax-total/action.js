...
import { Menu, Checkbox, DataGrid, Icon, LoadingMask } from 'edf-component'
...
class action {
	...
	load = async (msg) => {
		...
		let systemDate = await this.webapi.addTaxTotal.getSystemDate({})
		if(systemDate && !date) {
		...
		LoadingMask.show()
		let res = await this.webapi.addTaxTotal.query({ isFromOneself, beginYearMonth, endYearMonth })
		LoadingMask.hide()		
		if(res) {
	...
	update = async (msg) => {
		...
		LoadingMask.show()			
		let res = await this.webapi.addTaxTotal.query(getParmas)
		LoadingMask.hide()		
		if(res) {
    ...
    export = async () => {
		...
		let getParmas = this.getParmas()
		getParmas.querytype = 'accumulate'
        await this.webapi.addTaxTotal.export(getParmas)
    ...
    print = async () => {
        ...
        await this.webapi.addTaxTotal.print(getParmas)
    }
}
