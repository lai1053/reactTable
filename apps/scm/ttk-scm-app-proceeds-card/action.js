...
import { FormDecorator, Select, Checkbox, Form, DatePicker, Button, Input, Icon, ColumnsSetting, Popover, ShortKey } from 'edf-component'
...
class action {
	...
	initLoad = async (id) => {
		// id = 4574920094597120
		const response = await this.webapi.proceedsCard.init({ id: id })
		response.initId = id
		if (this.component.props.bankAccountId && !id) {
			response.bankAccountId = this.component.props.bankAccountId
		}
		let accounts = await this.webapi.proceedsCard.bankAccount({ entity: { isEnable: true }, status: true })
	...
	getDom = (e) => {
		const dom = document.querySelector('.ttk-scm-app-proceeds-card-form-details')
		if (!dom) {
			if (e) {
				return
			}
	...
	componentDidUpdate = () => {
		// console.log(this.metaAction.gf('data').toJS())
	}
	...
	componentDidMount = () => {
		...
		let thisStub = this
		setTimeout(() => {
			let dom = document.getElementsByClassName('ttk-scm-app-proceeds-card-form-header')[0] //ReactDOM.findDOMNode(thisStub.refs.auxItem)
			if (dom) {
	...
	handleKeyDown(e, index) {
		...
		if (e.key === 'Enter' || e.keyCode == 13 || e.keyCode == 108) {
			let dom = document.getElementsByClassName('ttk-scm-app-proceeds-card-form-header')[0]  //ReactDOM.findDOMNode(this.refs.auxItem)
	...
	getNextFocusIndex() {
		...
		if (nextFocusIndex == length + 1) {
			let dom = document.getElementsByClassName('ttk-scm-app-proceeds-card-form-details')[0]
			...
			setTimeout(() => {
				...
				setTimeout(() => {
					let a = $('.ttk-scm-app-proceeds-card-form-details').find('.ant-cascader-input')
					a.focus()
					a.click()
				}, 0)
			}, 16)
	...
	bodyKeydownEvent = (e) => {
		const dom = document.getElementById('ttk-scm-app-proceeds-card')
	...
	initOption = async () => {
		let obj = {}
		const list = [
			this.webapi.proceedsCard.customer({ entity: { isEnable: true } }),
			this.webapi.proceedsCard.supplier({ entity: { isEnable: true } }),
			this.webapi.proceedsCard.person({ entity: { isEnable: true } }),
	...
	checkDetailsOptionPersonal = ({ customerList, supplierList, personList, value, obj, index }) => {
		let list
		...
		const child = list.find(item => item.id == value.personId || item.id == value.customerId || item.id == value.supplierId)
		if (child) {
	...
	prev = async () => {
		...
		const response = await this.webapi.proceedsCard.previous({ code, isReturnValue: true })
		if (response) {
	...
	next = async () => {
		...
		const code = this.metaAction.gf('data.form.code')
		const response = await this.webapi.proceedsCard.next({ code, isReturnValue: true })
		if (response) {
			if (response.result == false && response.error) {
				this.metaAction.sfs({
	...
	handleResetSet = async (code) => {
		if (code) {
			const result = await this.webapi.proceedsCard.reInitByUser({ code: code })
	...
	handleConfirmSet = async (params) => {
		if (params) {
			...
			const result = await this.webapi.proceedsCard.updateWithDetail(setting)
			this.metaAction.sf('data.other.columnSetting', fromJS(result))
		}
	}
	...
	audit = async () => {
		...
		if (status == consts.consts.VOUCHERSTATUS_NotApprove || status == consts.consts.VOUCHERSTATUS_Rejected) {
			const response = await this.webapi.proceedsCard.audit({ id, ts })
			...
			const response = await this.webapi.proceedsCard.unaudit({ id, ts })
			this.injections.reduce('update', { path: 'data.other.isAuditEdit', value: true })
			if (response) {
	...
	del = async () => {
		...
		if (ret) {
			const response = await this.webapi.proceedsCard.delete({ id, ts })
	...
	save = async (isNew) => {
		...
		if (form.id || form.id == 0) {
			...
			const response = await this.webapi.proceedsCard.update(params)
			...
			params.isReturnValue = true
			const response = await this.webapi.proceedsCard.create(params)
			if (response && response.result == false) {
				this.metaAction.toast('error', response.error.message)
			} else {
	...
	deptPerson = async (value) => {
		let customer = this.webapi.proceedsCard.customer({ entity: { isEnable: true } }), supplier = this.webapi.proceedsCard.supplier({ entity: { isEnable: true } }), person = this.webapi.proceedsCard.person({ entity: { isEnable: true } })
		let customerList = await customer
		let supplierList = await supplier
		let personList = await person
		let arr
	...
	updateEnclosure = async (res) => {
		const result = await this.webapi.proceedsCard.updateEnclosure(res)
		return result
	}
	...
	renderFormContent = () => {
		...
		let arr = [
			...
			<FormItem className="ttk-scm-app-proceeds-card-businessDate" label='单据日期' required={true}>
				<DatePicker
					...
					disabled={this.getDisable() || !(this.getVoucherVisible())}
					getCalendarContainer={() => document.getElementsByClassName('ttk-scm-app-proceeds-card-businessDate')[0]}
	...
	getVoucherVisible = () => {
		let data = this.metaAction.gf('data').toJS(),
			...
			sourceVoucherType = other.sourceVoucherTypeId != consts.VOUCHERTYPE_Delivery
	...
	disabledScroll = () => {
		...
		const dom = document.querySelector('.ttk-scm-app-proceeds-card-Cascader')
		if (!dom) {
			return setTimeout(() => {
				this.disabledScroll()
			}, 300)
	...
	renderPopover = () => {
		return (
			<Popover content={(
				...
				<Icon type='jianpan' fontFamily='edficon' className='ttk-scm-app-proceeds-card-header-left-iconbutton'></Icon>
			</Popover>
		)
	}
}
