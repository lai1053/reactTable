...
class reducer {
	...
	initLoad = (state, response) => {
		...
		if (parsed.other) {
			...
			other = Object.assign(other, parsed.other);
	...
	load = (state, voucher) => {
		if (voucher) {
			...
			//如果行数太少,则用空行补足
			if (voucher.details) {
				const details = this.metaReducer.gf(state, 'data.form.details')
					.toJS();
	...
	parseResponse = (response, noDetails, state) => {
		...
		// for (let i = 0; i < response.businessTypes.length; i++){
		// 	arr = [...arr, ...response.businessTypes[i].children]
		// }
	...
	moveRowToUpOrDown = (state, gridName, rowIndex, upOrDown) => {
		let details = this.metaReducer.gf(state, `data.form.${gridName}`)
			.toJS();
