...
class action {
    ...
    onInit = ({ component, injections }) => {
        ...
        //console.log(this.metaAction.context);
    ...
    getVoucher = async () => {
        ...
        const deliveryIdList = [];
        //需要将选中的数据按照列表的顺序排序
        dataSource.forEach(item => {
            let sc = selectedOption.find(ele => ele.id == item.id);
            if (sc) {
                if (item.status != ARRIVAL_Approved && !item.discarded) {
                    flag = true
                    data.push(
                        {
                            id: item.id,
                    ...
                    if (item.customerId && customerIdList.indexOf(item.customerId) == -1) {
                        customerIdList.push(item.customerId);
                    }
                    if (deliveryIdList.indexOf(item.id) === -1) {
                        deliveryIdList.push(item.id);
                    }
                }
            }
        })
        ...
        if (getInvoiceInvMatch.allMatched !== true) {
            return await this.collateInvoice({ data, idList: deliveryIdList });
        }
    ...
    getVoucherToTJ = async () => {
        ...
        const deliveryIdList = [];
        let hasNoCustomer = false;//有客户id不存在的数据
        //需要将选中的数据按照列表的顺序排序
        dataSource.forEach(item => {
            let sc = selectedOption.find(ele => ele.id == item.id);
            if (sc) {
                if (item.status != ARRIVAL_Approved && !item.discarded) {
                    ...
                    if (!item.customerId) hasNoCustomer = true;
                    ...
                    // }
                    if (deliveryIdList.indexOf(item.id) == -1) deliveryIdList.push(item.id)
                }
            }
        })
        if (!flag) {
    ...
    tjInitSetting = async () => {
        ...
        // 	title: '初始化配置',
        // 	wrapClassName: 'tj-setting-card',
        // 	// width: 1200,
        // 	className: 'cards',
        // 	okText: '确定',
        // 	bodyStyle: { padding: '10px 0px 10px 24px' },
        // 	footer: '',
        // 	children: this.metaAction.loadApp('ttk-scm-tj-init-setting', {
        // 		store: this.component.props.store
        // 	}),
        // })
    ...
    incomeexpensesSettingTJ = () => {
        this.component.props.setPortalContent &&
            this.component.props.setPortalContent(
                '收支类型设置',
                'ttk-scm-incomeexpenses-setting-tj',
                { accessType: 1 }
            )
    }
    fileMapping = () => {
    ...
    supplyMapping = () => {
        this.component.props.setPortalContent &&
            this.component.props.setPortalContent(
                '供应商映射',
                'ttk-scm-app-supply-mapping',
                { accessType: 1 }
            )
    }
    tjsetting = async () => {
        const res = await this.metaAction.modal('show', {
            title: <div><span className='tj-setting-title'>设置</span><span className='tj-setting-title-tip'>温馨提示：发票和申报功能，是《方欣科技》金财管家组成部分</span></div>,
            width: 460,
            okText: '确定',
            footer: null,
            className: 'ttk-scm-tj-setting-container',
            children: this.metaAction.loadApp('ttk-scm-tj-setting', {
                store: this.component.props.store,
                vatOrEntry: 0
            }),
        })
    ...
    supplement = async () => {
        ...
        const selectedOption = this.metaAction.gf('data.tableCheckbox.selectedOption').toJS() //选中的
        let revenueType = this.metaAction.gf('data.other.revenueType')

        revenueType = revenueType && revenueType.toJS()
        if (selectedOption.length == 0) {
        ...
        newData.forEach(item => {
            if (item.status != ARRIVAL_Approved) {
                ...
                if (item.taxRateId == 9999 || item.taxRateId == 1000) {
                    taxRateId = 0
                }
                if (taxRate.indexOf(taxRateId) == -1) {
                    taxRate.push(taxRateId)
                }
            }
        })
        ...
        if (taxRate.length != 1) {
            taxRateFlag = 1
        } else if (taxRate[0] != 0 && taxRate[0] != 6) {
            taxRateFlag = 2
        }
    ...
    subManage = () => {
        this.component.props.setPortalContent &&
            this.component.props.setPortalContent('科目设置', 'edfx-business-subject-manage?from=sa', { accessType: 'sale' })
    }
    // 生成档案规则设置
    fileRules = async () => {
        const ret = await this.metaAction.modal('show', {
    ...
    collateInvoice = async ({ data = null, idList = null }) => {
        ...
        const response = await this.webapi.queryAccountEnable(param)
        let aaa = 'sa'
        if (response.isShow) {
            const resNew = await this.metaAction.modal('show', {
                title: '生成凭证设置',
    ...
    showTip = async (id, ts) => {
        const ret = await this.metaAction.modal('show', {
            ...
            width: 500,
            footer: false,
            wrapClassName: 'ttk-scm-app-collect-tip-body',
        ...
        const ret = await this.metaAction.modal('show', {
            ...
            children: this.metaAction.loadApp('ttk-scm-app-collect', {
                ...
                collectOnOk: async (params) => {
                    ...
                    // if (domainName[0] == 'localhost:8081' || domainName[0] == 'dev' || domainName[0] == 'test') {
                    const seq = await this.webapi.collecteData1({ ...params, vatOrEntry: 0 });
                    ...
                    // } else {
                    //     return await this.webapi.collecteData({ ...params, vatOrEntry: 0 });
                    // }
                },
                flag: 'sa'
            })
    ...
    onCollectResultModal2 = async (option) => {
        const ret = await this.metaAction.modal('show', {
            ...
            children: this.metaAction.loadApp('ttk-scm-app-collect-result', {
                ...
                issuedByTax: option.issuedByTax,
                flag: 'sa',
                vatOrEntry: 0,
    ...
    onCollectResultModal = async () => {
        const ret = await this.metaAction.modal('show', {
            ...
            children: this.metaAction.loadApp('ttk-scm-app-collect-result', {
                ...
                dateVisible: true,
                vatOrEntry: 0,
                enableddate: this.metaAction.gf('data.other.enableddate'),//启用日期
    ...
    getTableScroll = () => {
        try {
            ...
            let dom = document.getElementsByClassName('ttk-scm-app-sa-invoice-list-Body')[0]
            let tableDom
            if (!dom) {
                return
            }
    ...
    //渲染列
    renderColumns = () => {
        let columns = this.metaAction.gf('data.other.columnDto').toJS() //header
        ...
        columns.forEach(data => {
            ...
            if (sortColumns) {
                        ...
                        if (length == 0) {
                            ...
                            const obj = {
                                children: <span className="ttk-table-app-list-td-con">
                                    ...
                                    </Menu>} title="收款列表" placement='left'>
                                        <a
                                            href="javascript:;"
                                            className="table-needDel"
                                            onClick={() => this.openMoreContent(text[0].id, 'settleCodes')}
            ...
            else if (data.fieldName == 'customerName') {
                ...
                arr.push({
                    ...
                    onCell: (record) => {
                        ...
                        //是否为第一行
                        if (index == 0 && !record['customerName']) {
                            return {
                                style: {
                            ...
                            if (!record['customerName']) {
                                ...
                                //  console.log(rowSpan)
                                if ((index + rowSpan) < ds.length && !ds[index + rowSpan]['customerName'] && ds[index + rowSpan][checkboxKey]) {
                                    return {
                                        style: {
            ...
            //     })
            // } 
            else if (data.fieldName == 'revenueTypeName') {
                //收入类型
                arr.push({
                    title:
                        <span style={{ marginLeft: '-20.5px' }}>{data.caption}<Popover content='点 更多/批量修改，修改收入类型' placement='right' overlayClassName='ttk-scm-app-sa-invoice-list-helpPopover' >
                    ...
                    onCell: (record) => {
                        ...
                        const index = record.uuid;
                        //是否为第一行
                        if (index == 0 && !record.revenueTypeName) {
                            // if (!record.inventoryName) {
                            //     //共用borderLeft
                            //     return {
                            //         style: {
                            ...
                            if (!record.revenueTypeName) {
                                ...
                                //   }
                            } else if (index < ds.length - 1 && !ds[index + 1].revenueTypeName && ds[index + 1][checkboxKey]) {
                                return {
                                    style: {
                                        borderBottomColor: 'red',
                                    }
    ...
    calcRowSpan = (text, checkboxKey, currentRowIndex, list) => {
        ...
        //  console.log(list.getIn([1,'id'])); //获取list中第i个docId的值
        return rowSpan
    }
    ...
    needAlignType = (key) => {
        ...
        const left = ['remark', 'customerName', 'inventoryName', 'invoiceTypeName', 'specification', 'invoiceInvName', 'inventoryTypeName', 'invoiceCode', 'revenueTypeName']
        const center = ['seq', 'voucherSourceName', 'businessDate', 'code', 'invoiceTypeName', 'invoiceNumber', 'settleCodes', 'docCode', 'invoiceDate', 'customerName']
        ...
        const obj = {
            children: (
                record[checkboxKey] ? <span>
                    {(status == ARRIVAL_NotApprove || status == ARRIVAL_Rejected) && record.discarded == false && this.notMinfei() && <a
                        className={`${record.discarded == true ? 'disabled' : ''}`}
                        onClick={record.discarded == true ? () => { } : () => this.auditModal(record[checkboxKey], ts, record['customerId'], record['inventoryId'])}
                        href="javascript:;"
                    >
                    ...
                    <a
                        ...
                        onClick={status != ARRIVAL_NotApprove && status != ARRIVAL_Rejected ? () => { } : () => this.delModal(record[checkboxKey], ts)}
                        href="javascript:;"
                    >
    ...
    //生成凭证提示框
    auditModal = async (id, ts, customerId) => {
            ...
            if (columnSolution) {
                ...
                if (columnDetail) {
                    ...
                    if (this.metaAction.context.get("currentOrg").vatTaxpayer != 2000010002) {
                        ...
                        columnDetail.columnDetails[index].width = width + 33;
                    }
    ...
    searchValueChange = (value) => {
        ...
        //if (value.isDraft) value.isDraft = true
    ...
    sortParmas = (search, pages, order, type, noInitDate) => {
        ...
        let { customerName = null, invoiceTypeId = null, invoiceNumber = null, accountStatus = null, discarded, taxRateId = null, inventoryName = null, inventoryType = null, taxInclusiveAmountStr = null } = search;
        if (invoiceNumber == '') {
            invoiceNumber = null
        }
        ...
        // delete search.discarded
        const searchValue = utils.sortSearchOption(search, null, ['customerName', 'invoiceTypeId', 'invoiceNumber', 'accountStatus', 'discarded', 'taxRateId', 'inventoryName', 'inventoryType', 'taxInclusiveAmountStr']);
    ...
    initData = async (params) => {
        const customer = await this.webapi.customer();//查询供应商
        ...
        this.injections.reduce('tableLoading', false)
        this.injections.reduce('load', { response, isInit: true, customer: customer.list })
        setTimeout(() => {
            this.getTableScroll()
        }, 0)
    }
    ...
    requestData = async (params) => {
        ...
        this.injections.reduce('tableLoading', false)
        //   this.metaAction.sf('data.other.asc', params.orders[0].asc)
    ...
    openDocContent = async (id, text, name) => {
        if (id && name && name != '系统凭证') {
            ...
            this.metaAction.toast('error', `请在${name}管理查看生成的凭证`)
        } else {
            this.component.props.setPortalContent &&
                this.component.props.setPortalContent(
                    '填制凭证',
    ...
    // filterOptionSummaryold = (input, option, name) => {
    //     //  console.log(input,option,name)
