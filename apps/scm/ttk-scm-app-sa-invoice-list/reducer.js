...
class reducer {
    ...
    load = (state, { response, isInit, customer }) => {
        ...
        if (isInit) {
            const { invoiceTypes, accountStatuses, rateList, inventoryTypes } = response;
            state = this.metaReducer.sf(state, 'data.other.revenueType', fromJS(response.businessTypes))
            state = this.metaReducer.sfs(state, {
                ...
                'data.customer': fromJS(changeToOption(customer, 'name', 'id')),
                'data.inventoryTypes': fromJS(changeToOption(inventoryTypes, 'name', 'id'))
            })
        }
