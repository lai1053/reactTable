...
class action {
	...
	onResize = (type) => {
		...
		setTimeout(() => {
			if (this.keyRandom == keyRandom) {
				let dom = document.getElementsByClassName('ttk-scm-app-payment-list-Body')[0]
				if (!dom) {
					if (type) {
						return
					}
	...
	initOption = async () => {
		...
		console.log(params)
		const [response, account] = await Promise.all([this.webapi.payment.init(params), this.webapi.payment.bankAccount()])
	...
	sortParmas = async (search, page, order, type, noInitDate) => {
		...
		if (type != 'get') {
			let response = await this.webapi.payment.bankAccount(), account = [];
	...
	initData = async (params, noInitDate) => {
		...
		const response = await this.webapi.payment.init(params)
	...
	resizeEnd = async (params) => {
		...
		params.code = code
		let res = await this.webapi.payment.batchUpdate(params)
	...
	renderColumns = () => {
		const tableSetting = this.metaAction.gf('data.other.columnDetails').toJS()
		const tableOption = this.metaAction.gf('data.tableOption').toJS()
		const arr = []
		tableSetting.forEach(data => {
			...
			} else if (data.fieldName == 'amount' || data.fieldName == 'receivePayAmount') {
				arr.push({
					title: data.caption,
					key: data.fieldName,
					className: `table_td_align_${this.needAlignType(data)}`,
		...
		arr.push({
			title: (
				<Icon
					name="columnset"
					fontFamily='edficon'
					className='ttk-scm-app-payment-list-columnset'
	...
	rowSpan = (text, row, index, key) => {
		const obj = {
			children: <span className="ttk-scm-app-payment-list-td-con"><span
	...
	rowSpan2 = (text, row, index, fieldName) => {
		...
		const obj = {
			children: (
				<span className="ttk-scm-app-payment-list-td-con">
					<a href="javascript:;"
						onClick={() => {
	...
	normalTdRender2 = (text) => {
		return <span title={text} className="ttk-scm-app-payment-list-td-con" title={text}>{text}</span>
	}
	...
	showTableSetting = async ({ value, data }) => {
		...
		if (value === false) {
			...
			console.log('eeeeeee')
			const columnSolution = await this.webapi.payment.findByParam({ code: 'payList' })
			if (columnSolution) {
				let columnSolutionId = columnSolution.id
				const ts = this.metaAction.gf('data.other.ts')
				const columnDetail = await this.webapi.payment.updateWithDetail({
					"id": columnSolutionId,
	...
	auditClick = async (id, ts) => {
		const res = await this.webapi.payment.audit({ id, ts })
		if (res) {
	...
	allAuditClick = async () => {
		...
		const res = await this.webapi.payment.allAudit(this.delRepeat(data, 'id'))
		this.injections.reduce('tableLoading', false);
	...
	unauditClick = async (id, ts) => {
		const res = await this.webapi.payment.unaudit({ id, ts })
		if (res) {
	...
	allVersaAuditClick = async () => {
		...
		const res = await this.webapi.payment.allUnaudit(this.delRepeat(data, 'id'))
		this.injections.reduce('tableLoading', false);
		this.injections.reduce('update', { path: 'data.other.isAuditEdit', value: true })
		if (res) {
			if (res.fail && res.fail.length) {
	...
	delId = async (id, ts) => {
		const res = await this.webapi.payment.delete({ id, ts })
	...
	allDel = async () => {
		...
		const res = await this.webapi.payment.allDelete(this.delRepeat(data, 'id'));
		if (res) {
	...
	subjectManage = async () => {
		this.component.props.setPortalContent &&
			this.component.props.setPortalContent('科目设置', 'edfx-business-subject-manage?from=payment', { accessType: 'pay' })
	}
	// 凭证习惯
	voucherHabit = async () => {
	...
	export = async () => {
		...
		await this.webapi.payment.export(params)
	...
	print = async () => {
		...
		this.metaAction.modal('show', {
			...
			iconType: null,
			okText: '打印',
			className: 'mk-ttk-scm-app-payment-list-modal-container',
	...
	//
	// 	let _this = this
	// 	const {endDate} = this.metaAction.gf('data.searchValue').toJS()
	// 	this.metaAction.modal('show', {
	// 		title: '整理凭证号',
	// 		width: 320,
	// 		iconType: null,
	// 		className: 'mk-ttk-scm-app-payment-list-modal-container',
	// 		children: <SortProof time={endDate} callBack={_this.submitSortProof}/>,
	// 		footer: null
	// 	})
	// }
	...
	// 		'填制凭证',
	// 		'app-proof-of-charge',
	// 		{initData: {code: item.docCode, voucherDate: item.voucherDate}}
	// 	)
	// }
	//
	// getInsertItem = () => {
	...
	// 	this.component.props.setPortalContent &&
	// 	this.component.props.setPortalContent('填制凭证', 'app-proof-of-charge', {initData: {id: docId}})
	// }
	...
	// 		if (!mapData.has(item.code)) {
	// 			mapData.set(item.code, {code: item.code, ts: item.ts})
	// 		}
	// 	})
	// 	let data = []
	...
	resetClick = async () => {
		const code = this.metaAction.gf('data').toJS().other.code
		//重置栏目
		await this.webapi.payment.reInitByUser({ code: code })
		const columnSolution = await this.webapi.payment.findByParam({ code: 'payList' })
