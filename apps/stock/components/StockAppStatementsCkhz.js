...
class StockAppStatementsCkhz extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            loading: false, // 表格loading
            pageTitle: "出库汇总表", // 页面title
            tableOption: {
                // 表格定位对象
                x: "100%",
                y: 400,
    ...
    componentWillUnmount = () => {
        ...
        this[`deny-statements-ckhz-outportClickFlag`] = null
        this.props.saveLastData &&
            this.props.saveLastData(fromJS(this.state).toJS(), fromJS(this.params).toJS())
    }

    resizeTable = init => {
        const eleH = document.querySelector(".ttk-stock-app-statements-ckhz")
    ...
    load = async (isInitial, isTabLoad) => {
        ...
        if (resp) {
            if (
                ...
                dataList = resp.repOutInSummarySubDtoList.slice(0).map((item, i) => {
        ...
        this.setState({
            loading: false,
            list: dataList,
            // totalNum: utils.number.format(this.sum(resp.repOutInSummarySubDtoList, 'num'), 3),
            // totalBalance: utils.number.format(this.sum(resp.repOutInSummarySubDtoList, 'balance'), 2)
            totalNum:
    ...
    //     let inventoryList = await this.webapi.stock.getInventoryTypesFromArchives({ //存货科目
    //         "period": Period,
    //         "serviceCode": "FWGRK",
    //         "name": ""
    //     })
    ...
    getSearchParams = () => {
        var { id, name } = this.metaAction.context.get("currentOrg") || {}
    ...
    getTableScroll = e => {
        try {
            ...
            let appDom = document.getElementsByClassName("ttk-stock-app-statements-ckhz")[0] //以app为检索范围
    ...
    renderHeader = () => {
        ...
        return (
            <div className="ttk-stock-app-statements-ckhz-header">
                {/* <h2 className="ttk-stock-app-statements-ckhz-header-title">{pageTitle}</h2> */}
                <div className="ttk-stock-app-statements-ckhz-header-others">
                    <div className="ttk-stock-app-statements-ckhz-header-others-left">
                        <StatementsFilter
                            inputValue={code}
                            useInventory={{ value: inventoryIndex, orgId: orgId }}
                            useBusiness={{ value: businessIndex, inveBusiness, type: "outsummary" }}
                        ...
                        <MonthRangePicker
                            ...
                            className="ttk-stock-app-statements-ckhz-header-others-monthRangePicker"
                        />
                    </div>
                    <div className="ttk-stock-app-statements-ckhz-header-others-right">
    ...
    //     return (
    //         <div className="ttk-stock-app-statements-ckhz-footer">
    ...
    render() {
        ...
        return (
            <React.Fragment>
                ...
                {!isUnOpen && isVisible ? (
                    <div className="ttk-stock-app-statements-ckhz-main mk-layout">
                        {list && list.length ? (
                            <VirtualTable
                                className="ttk-stock-app-statements-ckhz-main-table mk-layout"
...
export default StockAppStatementsCkhz
