...
export default class action {
    ...
    priceChange = (rowIndex, rowData, v, isNCPFP) => {
        ...
        let amount= utils.number.round(rowData.amount, 6)
        let tax, taxInclusiveAmount
        if (quantity) {
    ...
    amountChange = (rowIndex, rowData, v, isNCPFP) => {
        ...
        let taxRate = rowData.taxRate,
            ...
            amount = formatNumbe(v),//解决删除时将小数点自动删除得问题
        ...
        if (taxRate || taxRate == 0) {
            ...
            if (quantity != 0) {
                ...
                if(_price<0){
                    _price=Math.abs(Number(_price))
                    quantity = utils.number.round(amount / _price, 6)
                }
            }else{
                if(price){
                    if(price<0){
    ...
    taxChange = (rowIndex, rowData, v, isNCPFP) => {
        ...
        let tax = formatNumbe(v), //解决删除时将小数点自动删除得问题
            // taxRate = utils.number.round(rowData.taxRate, 2),
            taxRate = rowData.taxRate,
            quantity = utils.number.round(rowData.quantity, 6),
            price = utils.number.round(rowData.price, 6),
            amount = rowData.amount?rowData.amount:utils.number.round(tax / taxRate, 2),
            taxInclusiveAmount = utils.number.round(amount + tax, 2)
        if (taxRate || taxRate == 0) {
            if (isNCPFP) {
                taxInclusiveAmount = utils.number.round(tax / taxRate, 2)
    ...
    sumColumn = (col, isPercent) => {
        ...
        if (currentSumCol == 'quantity') {
            let val= this.numberFormat(this.sum(details, (a, b) => a + b.get(`${currentSumCol}`)), 6)
            ...
            if(currentSumCol=='taxInclusiveAmount'||currentSumCol=='tax'||currentSumCol=='amount'){
                return this.sumColumnall(currentSumCol,details)
            }
            return this.numberFormat(this.sum(details, (a, b) => a + b.get(`${currentSumCol}`)), 2)
        }
    }
    sumColumnall= (currentSumCol, details) => {
        let list=details.toJS()
        let num=0
        if (!details || details.length == 0){
            return this.numberFormat(0, 2)
        }
        list.forEach(item=>{
            num=num+formatNumbe(item[currentSumCol])
        })
        return formatNumbe(num,2)?formatNumbe(num,2):this.numberFormat(0, 2)
    }
    sum = (details, fn) => {
        if (!details || details.length == 0)
            return this.numberFormat(0, 2)
