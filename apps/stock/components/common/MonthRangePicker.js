...
import { Message } from 'edf-component'
...
export default class MonthRangePicker extends PureComponent {
  ...
  onStartChange = value => {
    ...
    if(value.valueOf() > endValue.valueOf() || !value.isSame(endValue, 'year')) {
      !value.isSame(endValue, 'year') && Message.warning('不支持跨年查询')
  ...
  onEndChange = value => {
    ...
    if(value.valueOf() < startValue.valueOf() || !value.isSame(startValue ,'year')) {
      !value.isSame(startValue ,'year') && Message.warning('不支持跨年查询')
      this.onChange('startValue', value);
    }
  };
  ...
  render() {
    ...
    return (
      <div className={'ttk-stock-app-common-statements-month-range-picker' + ' ' + className} style={style}>
        <MonthPicker
