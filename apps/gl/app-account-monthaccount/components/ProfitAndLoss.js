...
import { Form, Radio, Button } from 'edf-component'
...
class InventoryCostingComponent extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      isAccountedMonth: props.isAccountedMonth || false,
  ...
  render() {
    let { profitAmount, lossAmount, docProduceModel, canEdit, isAccountedMonth } = this.state
    ...
    return (
      <div>
        ...
        <div className="btnContainer" style={{ PaddingRight: '10px', textAlign: 'right' }}>
          <Button className="btndisabled" disabled={!isAccountedMonth ? false : true} style={{ marginRight: '8px', fontSize: '12px', padding: '0px 15px' }} type='primary' onClick={this.confirm}>保存</Button>
