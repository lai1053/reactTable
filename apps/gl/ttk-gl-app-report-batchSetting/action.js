...
class action {
    ...
    getTableScroll = () => {
        try {
            ...
            let dom = document.getElementsByClassName('ttk-gl-app-report-batchSetting-root-tablediv-table')[0]
            let tableDom
            if (!dom) {
                return
            }
    ...
    handleSave = async () => {
        let { reClassType, valueType, showValueType , orgIds,dataSource,needCashFlowStatement} = this.metaAction.gf('data.other').toJS()
        // const res = await this.webapi.save(showValueType ? {reClassType, valueType} : {reClassType})
        const list = [
            this.webapi.save(showValueType ? { reClassType, dataSource,needCashFlowStatement,valueType, orgId: this.component.props.orgId || this.appParams.orgId || Number(orgIds[0])} : { reClassType, orgId: this.component.props.orgId || this.appParams.orgId || Number(orgIds[0]),dataSource,needCashFlowStatement}),
        ...
        if (res1) {
            ...
            if (result) {
                ...
                if(appVersion == 104){
                    ...
                    a.setAttribute('href', `Foresees://EtlParams:{"ConID":"CSY","orgID":"${this.component.props.orgId || this.appParams.orgId || Number(orgIds[0])}","AppID":"BB","FuncID":"GetKM"}`)
                }
                a.click()
            } else {
                 response = await this.webapi.saveGrade1AccountFromTTK({orgId: this.component.props.orgId || this.appParams.orgId || Number(orgIds[0])})
                 if(response.result == false && response.error != undefined){
                    return false
                }
            }
