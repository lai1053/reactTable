            ...
            cell={({ rowIndex, ...props }) => {
                const record = dataSource[rowIndex] || { [detailListName]: [] },
                    ...
                    lh = noShowDetailList ? 37 :
                        (lineHeight || 37) *
                        (totalRow ? rowDetailList.length + 1 : rowDetailList.length);
                return (
                    <Cell
                        {...props}
...
function datagridCheck() {
    ...
    dataInput[`datagridCheck`] = true

    clearCheck()
}


function clearCheck(wait = 500) {
    let dataInput = document.getElementById('data_input')

    if(!dataInput[`datagridCheck`]) return 
