...
export default class attachmentComponent extends Component {
                ...
                <div className={className + ' upload-content'}>
                    <Spin spinning={this.props.loading != undefined ? this.props.loading : false} tip='加载中...'>
                        <NoData type='noDocoments' small>
                            {this.props.status != 1 && <span>亲，还没有文件，赶快上传呦</span>}
                            {this.props.status == 1 && <span>暂无数据</span>}
                        </NoData>
                    </Spin>
                </div>
            )
                            ...
                            <div>
                                <Cell className='mk-attachment-content-link-cell'>
                                    ...
                                    {data[ps.rowIndex].type == 1000010001 || (data[ps.rowIndex].file && data[ps.rowIndex].file.type == 1000010001) ?
                                        ...
                                        <span title={data[ps.rowIndex].name || data[ps.rowIndex].alt || (data[ps.rowIndex].file &&data[ps.rowIndex].file.originalName)}>
                                            {data[ps.rowIndex].name || data[ps.rowIndex].alt || (data[ps.rowIndex].file &&data[ps.rowIndex].file.originalName)}</span>}
                                </Cell>
                            </div>
                    )
                }}
    ...
    render() {
        ...
        let popupContainerModal = this.props.popupContainerModal
        ...
        return (
            this.props.isFromDocList ?
                <Popover
                    ...
                    getPopupContainer={() => document.querySelector(popupContainerModal)}
