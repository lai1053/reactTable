...
class TableOperate extends PureComponent {
    ...
    render() {
        const { status, disable, submitNote, viewClick, editClick, deleteClick, noteClick, className, style ,settleAccounts, row} = this.props
        const showBtn = []
        ...
        if (status == 1) {
            ...
            if(settleAccounts){
                showBtn.push(
                    ...
                    href="javascript:;"
                >
                    <Icon fontFamily='edficon' type="shanchu" title="已月结不能删除" />
                </a>
                )
            }else{
                showBtn.push(
                ...
                <Popover content={
                    <div >
                        <Input.TextArea rows={7}
                            ...
                            onBlur={() => {this.setState({visible: false});submitNote(this.state.noteContent)}}
                    ...
                    <a
                        ...
                        className = {!row.note?'disabledNote':''}
