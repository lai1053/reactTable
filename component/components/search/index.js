...
class SearchComponent extends PureComponent {
    ...
    componentDidMount() {
        ...
        if(document.getElementsByClassName('mk-search-high-search') && Array.isArray(document.getElementsByClassName('mk-search-high-search'))) {
    ...
    componentWillUnmount() {
        ...
        if (dateDom) {
            dateDom.removeEventListener('click', this.rangePickerClick, false)
        }
    ...
    showMoreSearch = (bol) => {
        ...
        // TTK-18160
        if (bol) {
            const dom = document.querySelector('.mk-search-high-search')
            if (dom&&[...dom.classList].indexOf('slideOutUp')>-1) {
                dom.style.display='block'
            }
            this.showThis = true
        }
        this.props.queryAccountSubjects && this.props.queryAccountSubjects()
        this.setState({
            data,
            count: 1
        })
    ...
    cancelClick = () => {
        ...
        if (cancelClick) {
            setTimeout(() => {
                cancelClick( clonedeep(this.props.moreSearch))
            }, 800)
        }
    ...
    clearValue = (status) => {
        ...
        moreSearchItem.forEach(item => {
            if (item.range) {
                ...
                if(item.props&&item.props.accountingSubjectFlag){
                    let name = item.key?item.key.split('.')[item.key.split('.').length-1]:item.props.key
                    clearValue[name] = null
                }else{
    ...
    normalSearchChange = (name, e, type, bol) => {
        ...
        let normalSearchMeta = this.props.normalSearch
        let isImmediatelyChange // 检查查询组件的属性，代表值变更时是否立即执行简单查询（false：不执行，undefined和true：立即执行）

        for (var i = 0; i < normalSearchMeta.length; i++) {
            if (normalSearchMeta[i].name == name) {
                isImmediatelyChange = normalSearchMeta[i].isImmediatelyChange
            }
        }

        // 测试说简单搜索的情况下要清空高级搜索的内容
        setTimeout(() => {
            if (this.state.normalSearch[name] == value && this.props.normalSearchChange && isImmediatelyChange != false) {
                let initValue = this.clearValue()
                this.props.normalSearchChange(name, value, initValue, type)
            }
        }, 1000)
    ...
    renderNormalSearch = () => {
        ...
        return normalSearch.map((item, index) => {
            item.key = `normal-search-item-${index}`;
    ...
    // TTK-18160 解决tabs组件里面 当点击取消之后 切换tab回来刷出高级查询的问题
    handleAnimationEnd = () => {
        const dom = document.querySelector('.mk-search-high-search')
        if (dom && [...dom.classList].indexOf('slideOutUp')>-1) {
            // setTimeout(() => {
    ...
    render() {
        ...
        let This = this
         if(!this.showThis) {
            return (
                <div ref='retrieveWrap' className='retrieveWrap mk-search' style={{ position: 'relative' }} onAnimationEnd={() =>this.handleAnimationEnd()}>
        ...
        const moreSearchItem = this.props.moreSearchItem ? this.props.moreSearchItem.filter(item => {
            ...
            if (item.type != 'accountCodeTest'&&item.pre && item.next &&
            ...
            <div ref='retrieveWrap' className='retrieveWrap mk-search' style={{ position: 'relative' }} onAnimationEnd={() =>this.handleAnimationEnd()}>
                ...
                >
                    <div
                        ...
                        className={`mk-search-high-search ${this.state.data.get('childVisible') ? '' : 'slideOutUp'}`}
                    >
                        <h2>
                            <div>高级查询</div>
                        </h2>
