...
class TableSettingCard extends PureComponent {
    constructor(props) {
        super(props)
        this.state = {
            data: this.ininData(props.data),
            initData: JSON.parse(JSON.stringify(props.data == undefined ? '' : props.data)),
            height: null,
            top: -2000,
    ...
    renderItem = (data) => {
        ...
        data.forEach((item, index) => {
            if (item.isHeader == true) {
                arrLeft.push(
                    <div key={`mk-tableSetting-item-${index}`} className="mk-tableSetting-item">
                        <Checkbox
                ...
                arrRight.push(
                    <div key={`mk-table-setting-item-${index}`} className="mk-tableSetting-item">
                        <Checkbox
                            className={`${item.isVisible ? 'active' : ''}`}
                            checked={item.isVisible}
                            onChange={(value) => this.handleChange(value, item)}
    ...
    renderItem2 = (data) => {
        const arr = data.map((item, index) => {
            return (
                <div key={`mk-table-setting-item2-${index}`} className="mk-tableSetting-item-style2">
                    <Checkbox
                        className={`${item.isVisible ? 'active' : ''}`}
                        checked={item.isVisible}
                        onChange={(value) => this.handleChange(value, item)}
    ...
    render() {
        ...
        return (
            <div style={{ display: `${this.state.firstVisible == false ? 'none' : 'block'}` }}>
                ...
                >
                    <h2>栏目设置</h2>
                    {props.showTitle && <div className="mk-tableSetting-header">
                        <span>表头</span>
                        <span>明细</span>
                    </div>}
                    ...
                    <div
                        className={
                            classNames({
                                'mk-tableSetting-container-wrapper': true,
                    ...
                    <div className={className2}>
                        ...
                        <Button className="mk-tableSetting-bottom-btn cancle" onClick={this.cancelClick}>取消</Button>
                    </div>
                </div>
