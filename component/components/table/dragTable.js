...
class DragTable extends PureComponent {
    ...
    componentDidMount() {
  	...
  	//拖拽初始化及逻辑
  	draftSort = () => {
   	 	let el = document.getElementsByClassName('ant-table-tbody')[0];
        let self = this; //可忽略
    	let sortable = Sortable.create(el, {
      		animation: 50, //动画参数
            onEnd: function (evt) { //拖拽完毕之后发生，只需关注该事件
                self.props.moveRow(evt)
            }
   	 });
  }
...
DragTable.Column = Table.Column
DragTable.ColumnGroup = Table.ColumnGroup
DragTable.rowSelection = Table.rowSelection
DragTable.selection = Table.selection
