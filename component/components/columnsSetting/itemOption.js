...
class ItemOption extends PureComponent {
    constructor(props) {
        super(props)
        this.state = {
    ...
    itemClick = (key, e) => {
        ...
        let option2 = option.map(item => {
            return { ...item }
        })
        const index = option2.findIndex(item => item[singleKey] == key)
        if (index > -1) {
            option2[index][checkedKey] = e.target.checked
            this.setState({
                option: option2,
                checkAll: this.checkAllChecked(option2, checkedKey)
            })
            itemClick && itemClick(option2, option2[index])
        }
    }
