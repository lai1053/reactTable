...
export default class action {
    ...
    getInventory = async (params, field) => {
        ...
        if (response) {
            this.metaAction.sf(field || 'data.other.inventory', fromJS(this.getFullName(response.list)))
        }
    }
    getFullName = (list) => {
        list.map(item => {
    ...
    addInventory = async (field, type) => {
        let appId = sessionStorage['appId'], ret
        if(appId == 114){
            //新代账
    ...
    calc = (fieldName, rowIndex, rowData, params, isNCPFP,isHGSFP) => {
        let v = params.value
        if (fieldName === 'price') {//单价
            this.priceChange(rowIndex, rowData, v, isNCPFP,isHGSFP)
        }
        else if (fieldName === 'amount') {//金额
            this.amountChange(rowIndex, rowData, v, isNCPFP,isHGSFP)
        }
        else if (fieldName === 'quantity') {//数量
            this.quantityChange(rowIndex, rowData, v, isNCPFP,isHGSFP)
        }
        else if (fieldName === 'taxRateName') {//税率
            this.taxRateChange(rowIndex, rowData, v, params.hasChangeTaxAmount, isNCPFP,isHGSFP)
        }
        else if (fieldName === 'tax') {//税额
            this.taxChange(rowIndex, rowData, v, isNCPFP,isHGSFP)
        }
        else if (fieldName === 'taxInclusiveAmount') {
            this.taxInclusiveAmountChange(rowIndex, rowData, v, isNCPFP,isHGSFP)
        }
    ...
    taxRateChange = (rowIndex, rowData, v, hasChangeTaxAmount, isNCPFP,isHGSFP) => {
        // 金额＝价税合计÷（1＋税率）
        // 税额＝价税合计－金额
        // 如果数量为0，单价为0
        // 如果数量不为0:
        ...
        if (isNCPFP) {
            ...
            if(taxRate != 0) {
                ...
                tax = utils.number.round(0, 2)
                taxInclusiveAmount = utils.number.round(amount, 2)
            }
    ...
    taxChange = (rowIndex, rowData, v, isNCPFP) => {
        ...
        if (taxRate || taxRate == 0) {
            ...
            //******start****** 2019-11-14 新需求JCFINTAX-1181 修改税额只反算价税合计， 价税合计 = 金额 + 税额
            taxInclusiveAmount = utils.number.round(rowData.amount + tax, 2)
            this.metaAction.sfs({
                [`data.form.details.${rowIndex}.tax`]: tax,
                [`data.form.details.${rowIndex}.taxInclusiveAmount`]: taxInclusiveAmount
            })
            ...
            //*******end*******2019-11-14 新需求JCFINTAX-1181 修改税额只反算价税合计， 价税合计 = 金额 + 税额
    ...
    sumColumn = (col, isPercent, decimals) => {
        let currentSumCol = col,
            details = this.metaAction.gf('data.form.details')
        if (currentSumCol == 'quantity') {
            let val= this.numberFormat(this.sum(details, this.sumfn(currentSumCol)), decimals?decimals:6)
            ...
            return `${this.numberFormat(this.sum(details, this.sumfn(currentSumCol)), decimals?decimals:2)}%`
        } else {
            return this.numberFormat(this.sum(details, this.sumfn(currentSumCol)), decimals?decimals:2)
        }
    }

    sumfn  =  (currentSumCol)  =>  (a,  b)  =>  {
        let  v  =  b.get(`${currentSumCol}`);
        v  =  Number(`${v}`.replace(/,/g,  ''))
        return  a  +  v
    }

    sum  =  (details,  fn,  currentSumCol)  =>  {
    ...
    checkSaveInvoice = (form, page, other) => {
        ...
        if (page == 'pu') {
            if (!form.supplierId && !form.supplierName && form.invoiceTypeId != 4000010047) {
                msg.push('销方名称不能为空')
            }
        } else if (page == 'sa') {
        ...
        if (page == 'pu' && !form.authenticated && other.vatTaxpayer == '2000010001' && (form.invoiceTypeId != 4000010030 && form.invoiceTypeId != 4000010047 && form.invoiceTypeId != 4000010010 && form.invoiceTypeId != 4000010900)) {
        ...
        if (page == 'pu' && form.authenticated && other.vatTaxpayer == '2000010001' && (form.invoiceTypeId != 4000010030 && form.invoiceTypeId != 4000010047 && form.invoiceTypeId != 4000010010 && form.invoiceTypeId != 4000010900)) {
            if (this.metaAction.context.get("currentOrg").vatTaxpayerChangeState == 0) {
        ...
        if (form.invoiceNumber && (form.invoiceNumber.length != 8) && (form.invoiceTypeId != 4000010040) && (form.invoiceTypeId != 4000010047)) {
